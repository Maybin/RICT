#--- ###########################################
#--- title: "RICT IV All INDICES SUMMER, SPRING AND AUTUMN ONLY version 1"
#--- output: html_document
#--- Author: Dr K. M Muyeba aka Maybin 
#--- #########################################

  ####################################################################################################################################
  # This R Script is a prediction engine for ALL INDICES in RICT IV 
  # Initially there are 80 indices (including) BMWP, LIFE,PSI, ePSI etc
  # 
  #################################################################################################################################### 
...
path <- "C:/DEFRA/Phase2/RICT"
setwd(path)
resultsPath <- "C:/DEFRA/Phase2/RICT/RStudio/AllIndices/Results"

source(paste0(path,"/supportfunctions/MeanAirTempAirTempRangeASFunction_new.R"))
source(paste0(path,"/supportfunctions/HelperFunctionsv1.R"))
source(paste0(path,"/supportfunctions/PredictionfunctionsV2.R"))
...
raw.input.data <- readCSVFile(paste0(path, "/datafiles/New Input file wValidation wTestData_12sites.csv"))
...
#Above loop same as # 
     #valid_alkalinity <- rbind(valid_alkalinity,(with(raw.input.data, mapply(getLogAlkalinity, HARDNESS, CALCIUM,     
     #CONDUCTIVITY, ALKALINITY, 1.2, 366))))
...
#new
AirTempGrid <- readCSVFile(paste0(path, "/datafiles/AirTempGrid.csv"))
...
write.csv(Warnings_file, file = paste0(resultsPath,"/Warnings_file_data.csv"))
...
write.csv(this_warning, file = paste0(resultsPath,"/Warnings_file_data.csv"))
...
writeToFile (AllFails_Warings, resultsPath, "/FinalAllFails_Warings.csv") 
...
writeToFile (failedRows, resultsPath, "/FinalFailingSites.csv") 
...
DFMean_gd <- read.delim(paste0(path,"/datafiles/DFMEAN_GB685.DAT"), header = FALSE, sep="", as.is=TRUE)
DFCoeff_gb685 <- read.delim(paste0(path,"/datafiles/DFCOEFF_GB685.DAT"), header = FALSE, sep="", as.is=TRUE)
...
#new
NRefg_groups <-  readCSVFile(paste0(path, "/datafiles/EndGrp_AssessScores.csv"))
...
filepathname <- paste0(path,"/datafiles/x103EndGroupMeans(FORMATTED)(v.JDB17Dec2019).csv") # x103EndGroupMeans(FORMATTED).csv")

# new use fread and data table
endgroup_IndexFrame <- getEndGroupMeans_dtableCopy (filepathname, model) # Change in AZURE, allIndices needs 80 indices, not just WHPT ??? change function in PredictionfunctionsV2.R ??
...
endgroup_IndexFrame <- filter(endgroup_IndexFrame, SeasonCode %in% SEASONS_TO_RUN) # CORRECT allIndices

# Select the indices to run, remove columns 1,2,3 and 84
indices_to_run <- names(endgroup_IndexFrame)[c(-1,-2,-3)] #CHANGE THESE TO 80 allIndices - 

        # This step below gives you the indices to run on season==1
        allIndices_values <- filter(endgroup_IndexFrame,SeasonCode==1)[,indices_to_run]
        # Multiply these with ProbScores, maybe write a new getSeasonIndexScoresV2()

# Run the index Scores   
mainData <- getSeasonIndexScores_new (final.predictors_try2, SEASONS_TO_RUN, indices_to_run, endgroup_IndexFrame, model) # Change in AZURE
...
biologicalData <- biologicalData[,-1]
      ...
      # We dont need to bind as there is no classification
      #mainData <- cbind(mainData, biologicalData)
writeToFile (mainData, resultsPath, "/FinalPredictions_AllIndices_data.csv") 

end_time <- proc.time()
print(" Time diff =")
print(end_time-start_time)

#NEXT - Tomorrow All raw values are correct *************
