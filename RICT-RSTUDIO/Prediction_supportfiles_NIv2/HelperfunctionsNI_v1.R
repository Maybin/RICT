...
# Helper functions for RIVPACS III+ for NI
...
# Produce dataframe with value of alkalinity in bounds [2.5, 366] as warning, and a message "Succ" or "Fail" to remove record from dataset for prediction
...
# This function is generic to these input environment variables: 
    ...
    # MEAN_DEPTH [1.7, 300]
    # SLOPE [0.1, 50]
    # DIST_FROM_SOURCE [2.2, 75]
    # DISCHARGE [0,10]
#  Use "NI (108 Group level?)
#  Check if it is "Fail" or "Warn", substr(getValidEnvInput(0)[2][[1]], 1,4)

getValidEnvInput <- function(env_var, lower_b, upper_b, envName) { # Add LONGITUDE, LATITUDE to this or declare another similar function with doubl;e bounds
  # lower_b <- 1
  # upper_b <- 590
  msg <- "Succ"
...
# 3. Function getAltitude(), change to base 10, Use "New NI (11-Group Level)". This function has been generalised to the 
# function getValidEnvInput
# The bounds are different: Warn [3,180], Fail [0,1345]. Value is less than 0 or greater than 1345, in which case "Fail".
# Value <1 or value >590, in whihc case "Warn"
# if value is 0-3 or 180-1345, "Warn".
# CASE 1: 0<x<3 or 180<x<1345, msg = "Warn"
# CASE 2: 180<x<1345, msg = "Fail"
  ...
  if(is.null(alt) | is.na(alt)) { # Null cant be evaluated, return log10(0)
    ...
    return (alt_frame)
      ...
      if(alt<lower_lowb | alt>upper_highb){ # Completely out of bounds, should fail with null value for "alt"
        ...
        alt_frame <- data.frame(log10(0), msg)
        colnames(alt_frame) <- c("_log","_msg")
        return (alt_frame)
      }
      else {
...
# Check that the input files - bould_cob+pebbpes_gr+snd+silt_cl=[0,100], and MSUBST = [-8,8], the latter is hard coded
...
getBNG <- function (nat_grid_ref, easting, northing,coordsys_bng) {
  lat_long    <- NA
  concat_bng  <- paste(nat_grid_ref, substr(easting,1,3), substr(northing,1,3), sep="") 
  new_bng    <- osg_parse (paste0("I",concat_bng), CoordSystem = coordsys_bng)
  return (new_bng)
}

# 12. sPECIAL CASE OF discharge from VELOCITY 
# Note the bounds for Discharge are [1,10], and bounds for velocity ar [1]
