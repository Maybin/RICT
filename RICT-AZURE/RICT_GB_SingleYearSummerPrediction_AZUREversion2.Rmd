...
taxaabDFrame <- read.csv("src/TAXAAB.csv")

EndPoints <- read.csv("src/Test_Data_End_Point_Means_Copy.csv")
...
NRefg_groups <- read.csv("src/EndGrp_AssessScores.csv")
...
SEASONS_TO_RUN <- c(raw.input.data$SUM_SEASON_ID[1]) # change AZURE, SUMMER
...
mainData <- getSeasonIndexScores (final.predictors_try3, SEASONS_TO_RUN, indices_to_run, endgroup_IndexFrame, model) # Changed AZURE
 
#Remove failing sites from biologicalData
#biologicalData <- biologicalData[!(biologicalData$SITE %in% this_failing$SITE),]
biologicalData <- biologicalData[is.na(match(biologicalData$SITE, this_failing$SITE)), ]
