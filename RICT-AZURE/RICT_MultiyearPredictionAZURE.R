...
########################################################################
# This experiment is for multi-year prediction during spring and autumn#
# #######################################################################
...
source("src/MeanAirTempAirTempRangeASFunction_new.R")
...
coordsystem = "WGS84"
lat.long <- with(raw.input.data, getLatLong_AZURE(NGR,EASTING, NORTHING,coordsystem ) )
...
for(i in 1:nrow(raw.input.data)){
   valid_longitude <- rbind(valid_longitude,getLongitude(raw.input.data$LONGITUDE [i], -9, 1.5)) # Used -8, 1.4)) 
}
...
endgroup_IndexFrame <- getEndGroupMeans_dtable ("src/x103EndGroupMeans(FORMATTED).csv", model) # Change in AZURE
...
outputMainData <- cbind(mainData, biologicalData)
# head(mainData,7)

print("Time = ")
print(proc.time()-start_time)
maml.mapOutputPort("outputMainData")
