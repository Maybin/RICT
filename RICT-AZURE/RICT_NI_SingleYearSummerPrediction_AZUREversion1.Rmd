 #####################################################################################  
# Title : "RICT NI Model Single Season - Summer single Year Prediction Version 1"
...
source("src/PredictionfunctionsNI_V1.R")
source("src/HelperfunctionsNI_v1.R")
...
valid_altitude <- data.frame(log=as.numeric(), msg=as.character())
 for(i in 1:nrow(raw.input.data)){
   ...
   valid_altitude <- rbind(valid_altitude,getAltitude(raw.input.data$ALTITUDE [i], 3, 180)) # 1,590? 
 }
 colnames (valid_altitude) <- paste0("vld_alt_src",noquote(colnames(valid_altitude))) # vld = valid
...
for(i in 1:nrow(raw.input.data)){
  valid_discharge <- rbind(valid_discharge, getLogDischarge(raw.input.data$MEAN_DEPTH[i], raw.input.data$MEAN_WIDTH[i], raw.input.data$DISCHARGE [i], raw.input.data$VELOCITY[i],1, 10))
}
...
for(i in 1:nrow(raw.input.data)){
   valid_longitude <- rbind(valid_longitude,getLongitude(raw.input.data$LONGITUDE [i], -8, -5.7)) 
}
...
for(i in 1:nrow(raw.input.data)){
   valid_latitude <- rbind(valid_latitude,getLatitude(raw.input.data$LATITUDE [i], 54, 55.2)) #50.8, 52
}
